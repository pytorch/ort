...
import torch
import torch_ort as C
from .onnxruntime_inference_collection import get_ort_device_type

import onnx
from torch.utils.dlpack import from_dlpack, to_dlpack
from torch_ort import ort_to_dlpack

def torch_tensor_to_dlpack(tensor):
    if tensor.device.type == 'ort':
        return ort_to_dlpack(tensor)
    else:
...
class TrainingManager(GraphExecutionManager):
    ...
    def execution_session_run_forward(execution_session, onnx_model, device, *inputs):
        ...
        user_outputs = tuple(_utils._ortvalue_to_torch_tensor(forward_output, device) for forward_output in forward_outputs)
