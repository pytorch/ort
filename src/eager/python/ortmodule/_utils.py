...
from torch_ort import ort_from_dlpack
...
def _ortvalue_to_torch_tensor(ortvalue, device):
    ...
    torch_tensor = from_dlpack(dlpack_tensor) if device.type != 'ort' else ort_from_dlpack(dlpack_tensor)
...
def _torch_tensor_from_dl_pack(dlpack, ortvalue, device):
    torch_tensor = from_dlpack(dlpack) if device.type != 'ort' else ort_from_dlpack(dlpack)
    return torch_tensor.to(torch.bool) if ortvalue.data_type() == 'tensor(bool)' else torch_tensor
